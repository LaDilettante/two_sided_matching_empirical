...
for (filename in c("xx_nojobs.feather", "ww_nojobs.feather",
                   "true_opp_nojobs.feather", "obs_opp_nojobs.feather",
                   "choice_nojobs.feather",
                   "true_alpha_nojobs.feather", "true_beta_nojobs.feather")) {
  varname <- stringr::str_split(filename, "_nojobs", simplify = TRUE)[1]
  res <- as.matrix(read_feather(filename))
  dimnames(res) <- NULL
  assign(varname, res)
}
choice <- choice + 1 # offset python 0-based index
